...
namespace Core.CrossCuttingConcerns.Serilog.Loggers
{
    public class FileLogger : LoggerServiceBase
    {
        ...
        public FileLogger(IConfiguration configuration)
        {
            ...
            FileLogConfiguration logConfig = configuration.GetSection("SerilogLogConfigurations:FileLogConfiguration").Get<FileLogConfiguration>()??throw new Exception(SerilogMessages.NullOptionsMessage);

            string logFilePath = string.Format(format: "{0}{1}",
                arg0: Directory.GetCurrentDirectory() + logConfig.FolderPath, arg1: ".txt");

             Logger = new LoggerConfiguration().WriteTo.File(logFilePath,
